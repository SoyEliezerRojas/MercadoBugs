...
alter table public.orders
add constraint orders_checkout_request_id_unique unique (checkout_request_id);
...
alter table public.orders
add constraint orders_shipping_name_length
check (char_length(btrim(shipping_name)) between 2 and 100);

alter table public.orders
add constraint orders_shipping_address_length
check (char_length(btrim(shipping_address)) between 5 and 200);

alter table public.orders
add constraint orders_shipping_city_length
check (char_length(btrim(shipping_city)) between 2 and 100);

alter table public.orders
add constraint orders_shipping_postal_code_length
check (char_length(btrim(shipping_postal_code)) between 2 and 20);

-- Cart status and ownership are controlled by the checkout RPC once a cart exists.
revoke update (user_id, status) on public.carts from authenticated;

create function private.checkout_shipping_cost(shipping_method text)
returns numeric
language sql
immutable
set search_path = ''
as $$
  select case shipping_method
    when 'standard' then 5.00::numeric(12, 2)
    when 'express' then 12.00::numeric(12, 2)
    else null::numeric(12, 2)
  end;
$$;

revoke all on function private.checkout_shipping_cost(text) from public, anon, authenticated;

create function public.manage_cart_coupon(
  action text,
...
begin
  ...
  if normalized_action = 'remove' then
    ...
    elsif selected_coupon.expires_at is not null and validation_time > selected_coupon.expires_at then
  ...
  if selected_coupon.id is not null then
    discount_amount := case selected_coupon.discount_type
      when 'percentage' then round(subtotal_amount * selected_coupon.discount_value / 100, 2)
...
revoke all on function public.manage_cart_coupon(text, text, text) from public, anon, authenticated;
grant execute on function public.manage_cart_coupon(text, text, text) to authenticated;

create function public.perform_checkout(
...
begin
  ...
  if not found then
    -- A concurrent retry may have waited for the first request to convert the cart.
    select orders.*
    into existing_order
    from public.orders
    where orders.checkout_request_id = perform_checkout.checkout_request_id;
  ...
  set stock = products.stock - cart_items.quantity
  from public.cart_items
  where cart_items.cart_id = active_cart.id
...
revoke all on function public.perform_checkout(uuid, text, text, text, text, text, text)
from public, anon, authenticated;
grant execute on function public.perform_checkout(uuid, text, text, text, text, text, text)
to authenticated;
